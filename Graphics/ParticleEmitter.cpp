...
void ParticleEmitter::Initialise(unsigned int _maxParticles, unsigned int _emitRate,
	float _lifetimeMin, float _lifeTimeMax, float _velocityMin,
	float _velocityMax, float _startSize, float _endSize,
	const glm::vec4& _startColor, const glm::vec4& _endColor)
{
	...
	// Stores the variables that we pass in
	...
	// There needs to be four verices per particle to make a quad
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_maxParticles * 4 * sizeof(unsigned int),
...
{
	// Check is there are any avaliable particles for the system to emit
	if (m_firstDead >= m_maxParticles)
		return;
	
	// Return a dead particle to our avaiable list
...
void ParticleEmitter::Update(float _dt, const glm::mat4& _cameraTransform)
{
	...
	for (unsigned int  i = 0; i < m_firstDead; i++)
	{
		...
		{
			// This will need to be changed to use gravity (FYI)
			...
			// Finally to set up the quad using the correct position, color anmd scale/
